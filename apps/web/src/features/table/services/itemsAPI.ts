...
import { Item, ItemsPage, ItemsResponse } from "../types";

export const getAllItems = async ({
  cursor,
  fetchSize = 50,
}: { cursor?: number | null; fetchSize?: number } = {}): Promise<ItemsPage> => {
  const params: { fetchSize: number; cursor?: number } = { fetchSize };
  if (typeof cursor !== "undefined" && cursor !== null) params.cursor = cursor;
  const res = await httpService.get(`/items`, { params });
  const body = res.data as ItemsResponse;
...
export const updateItem = async (id: number, patch: Item) => {
