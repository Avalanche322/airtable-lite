...
const useUpdateMutation = () => {
  ...
  const updateMutation = useMutation<any, any, any, any>({
    mutationFn: (variables: { id: number; patch: Item }) =>
      updateItem(variables.id, variables.patch),
    onMutate: async (variables: { id: number; patch: Item }) => {
          ...
          items: (page.items || []).map((it: any) =>
            it.id === id
              ? {
                  ...it,
                  ...patch,
                  __pending: { baseVersion: it.version ?? 0, patch },
                  __conflict: false,
                }
              : it,
          ),
            ...
            items: (page.items || []).map((it: any) => {
              ...
              if (pending) {
                ...
                const applied = Object.keys(pendingPatch).every((k) => {
                  return (
                    (returnedItem as any)[k] === pendingPatch[k] ||
                    (returnedItem.data &&
                      returnedItem.data[k] === pendingPatch[k])
                  );
                });
                if (applied) {
